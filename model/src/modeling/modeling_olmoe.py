...
from .custom_models import RouterCompoundFast
...
class OlmoeSparseMoeBlock(nn.Module):
    def __init__(self, config):
        ...
        # gating
        if hasattr(config,'bigger_size') and config.bigger_size>=1:
            self.complex_gate=True
            self.gate = RouterCompoundFast(config)
        else:
            self.gate = nn.Linear(config.hidden_size, self.num_experts, bias=False)
            self.complex_gate=False
        ...
        self.max_topk=32 # 默认值，会被下面更新
        try:
            self.use_mask=config.use_mask
        except:
            self.use_mask=False
        if self.use_mask:
            self.max_topk=config.max_topk # 设原模型的topk*4
            self.gate_mask = nn.Parameter(torch.tensor(range(self.max_topk))-self.max_topk/2) 
            self.tau = 1
            self.entropy_loss_coef = 1e-2
            self.gate_mask_l2_coef = 1e-3
        try:
            self.use_list=config.use_list
        except:
            self.use_list=False
        if self.use_list:
            self.selection_mask = torch.tensor(config.fixed_list)
            self.max_topk=max(config.fixed_list)+1 # 设list里查topk最大的值
        

    def softTopK(self, x:torch.Tensor) -> torch.Tensor:
        x_norm = (x - x.mean()) / (x.std() + 1e-9)
        kth_val, _ = torch.kthvalue(-x_norm, self.top_k)
        threshold = -kth_val
        soft_y = torch.sigmoid(self.tau * (x_norm - threshold.detach()))
        soft_y = soft_y/torch.sum(soft_y,dim=-1)
        entropy_loss = -torch.sum(soft_y * torch.log(soft_y + 1e-9))
        return soft_y*self.top_k, entropy_loss
        # 和为top_k，数值表示概率，topK的部分的概率>0.5，非topK的部分的概率<0.5
        # 随着tau逐渐接近0，soft_y的选择逐渐变成只有topK的部分的数值为1，非topK的部分数值为0
    ...
    def forward(self, hidden_states: torch.Tensor) -> torch.Tensor:
        ...
        if self.complex_gate:
            router_logits = None # 不能直接做训练了
            ...
            if self.use_mask:
                routing_weights, selected_experts = torch.topk(routing_weights, self.max_topk, dim=-1)
                if self.training:
                    soft_y, entropy_loss = self.softTopK(self.gate_mask)
                    routing_weights = routing_weights*soft_y
                    aux_loss = entropy_loss * self.entropy_loss_coef 
                    l2_penalty = torch.sum(torch.pow(self.gate_mask, 2))
                    aux_loss += l2_penalty * self.gate_mask_l2_coef
                    # 选32个，软路由
                else:
                    _, top_k_indices = torch.topk(self.gate_mask, self.top_k, dim=-1)
                    routing_weights = routing_weights[:, top_k_indices] #与token无关的mask才可以这样直接选
                    selected_experts = selected_experts[:, top_k_indices]
                    # 硬路由到topk个
            elif self.use_list:
                sorted_weights, sorted_indices = torch.topk(routing_weights, self.max_topk,dim=-1) # 已经有序
                routing_weights = sorted_weights[..., self.selection_mask[:self.top_k]]
                selected_experts = sorted_indices[..., self.selection_mask[:self.top_k]]
            else:
        ...
        final_hidden_states = final_hidden_states.reshape(batch_size, sequence_length, hidden_dim)*self.routed_scaling_factor
...
class OlmoeModel(OlmoePreTrainedModel):
    ...
    def forward(
        ...
        get_gate_mask_loss = self.layers[1].mlp.use_mask # get if we are using gate_mask
        ...
        all_gate_mask_losses = () if get_gate_mask_loss else None
        ...
        for decoder_layer in self.layers[: self.config.num_hidden_layers]:
            ...
            if output_router_logits and layer_outputs[-1] is not None:
                if not get_gate_mask_loss:
                    all_router_logits += (layer_outputs[-1],)
                else:
                    all_router_logits += (layer_outputs[-1][0],)
                    all_gate_mask_losses += (layer_outputs[-1][1],)
        ...
        model_outputs=MoeModelOutputWithPast(
        ...
        if get_gate_mask_loss:
            model_outputs.gate_mask_losses = all_gate_mask_losses
...
class OlmoeForCausalLM(OlmoePreTrainedModel, GenerationMixin):
    ...
    def forward(
        ...
        **loss_kwargs,
        ...
        if return_dict and hasattr(outputs, "gate_mask_losses") and outputs.gate_mask_losses:
            # It's a tuple of scalar tensors, one per layer. Aggregate them.
            gate_losses = torch.stack(outputs.gate_mask_losses)
            entropy_loss = gate_losses.mean() 
