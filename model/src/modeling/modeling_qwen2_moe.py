...
class Qwen2MoeSparseMoeBlock(nn.Module):
    def __init__(self, config, layer_index):
        ...
        if hasattr(config,'bigger_size') and config.bigger_size>=1:
            ...
            self.gate = RouterCompoundFast(config, layer_index)
        else:
    ...
    def forward(self, hidden_states: torch.Tensor) -> torch.Tensor:
        ...
        #     #     for j in reversed(range(len(selected_experts[0]))):
        #     #         mapped = self.mapping[int(selected_experts[i][j])]
        #     #         if mapped == -1:
        #     #             # 没法del，至少对于vllm来说没法对齐，且tensor得是一样大
        #     #             selected_experts[i][j]=0
        #     #             routing_weights[i][j]=0
        #     #         else:
        #     #             selected_experts[i][j]=mapped
        #     mapped_experts = self.mapping[selected_experts.long()]
        #     mask = (mapped_experts == -1)
        #     selected_experts = torch.where(mask, 0, mapped_experts)
